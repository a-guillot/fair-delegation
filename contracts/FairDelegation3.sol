...
contract FairDelegation3 {
    ...
    function customer(address sp, bytes32 pk, bytes32 pk2) public payable {
        transactions[sp] = Transaction(pk, pk2, msg.value);
        emit CustomerTrigger(pk, pk2, msg.value);
    }

    function serviceProvider(bytes32 skSp) public payable {
        if (pair(transactions[msg.sender].pkSp,
                 transactions[msg.sender].pkSp2,
                 skSp)) {
            msg.sender.transfer(transactions[msg.sender].value);
